...
export function ExploreTab({ filteredBars, likes, likeCount, onLike, onBarClick }: ExploreTabProps) {
  ...
  useEffect(() => {
    ...
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      ...
      maxZoom: 19,
      minZoom: 10,
      className: 'leaflet-tiles-light'
    }).addTo(map)

    // Create custom pane for user marker
    map.createPane('userPane')
    map.getPane('userPane')!.style.zIndex = '9999'

    // Blue dot with white circle
    L.circleMarker(USER_POSITION, {
      radius: 10,
      fillColor: '#4285F4',
      color: '#ffffff',
      weight: 3,
  ...
  useEffect(() => {
    ...
    barMarkersRef.current.forEach(marker => {
      if (mapRef.current) {
    ...
    barsToDisplay.forEach((bar) => {
      ...
      const coords = BAR_COORDS[bar.id]
      if (!coords) return

      const iconColor = bar.currentStatus.offreActive ? COLORS.accent : COLORS.primary
      ...
      const circle = L.circleMarker(coords, {
        ...
        .bindPopup(`<strong>${bar.name}</strong><br/>${bar.location}`)
        .on('click', () => setSelectedBarOnMap(bar))
        .addTo(mapRef.current)

      barMarkersRef.current.push(circle)
    })
  }, [barsToDisplay, mapRef.current])
